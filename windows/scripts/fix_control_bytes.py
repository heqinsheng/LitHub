...
# ── 控制台编码兜底 ────────────────────────────────────────────────────
# Windows 中文控制台的 Python 默认编码是 cp936：print 出的上下文里若含 Å / ö / Π
# 会抛 UnicodeEncodeError，输出断在半路。只放宽错误策略、不改 encoding——编不出来
# 时退化成 "?"，UTF-8 环境下的输出字节一个都不变。本脚本不 import 任何本地模块，
# 所以自带一份（与 zapi.py 同款）。
