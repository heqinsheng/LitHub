...
| 空目录 | 库里没有任何条目归属、**且没有子目录**的分类目录 | 硬删除（跳过归档子树与 taxonomy 目录，见下） |
...
⚠️ 两条 2026-09-21 修掉的坑，都属「会丢东西」那一类：

- **「同一份文件」的判定不看笔记**。笔记没有文件名、`path` 也是 `None`，旧写法把每条笔记的
  签名都算成同一个 `("note","","")`，于是重复条目的笔记会被当成「同名子项」随条目一起
  **硬删**（正文只留在 `tidy_backup` 里）。
- **只要有子项没搬走，就整条跳过**（不删重复条目、不并标签），并把失败计入退出码。
  旧写法 `if not ok: continue` 只跳过当前那个子项，循环结束照删父条目——「搬不动」的后果
  是父条目消失而子项无人认领。`--apply` 也从此在**有失败时退出码非 0**（此前恒 0，
  systemd 与脚本串联看不出成败）。
...
与**`apply_to_zotero.py` 自己会建出来的目录**——它每轮把所有一级分类、全部子类与
`extra_categories` 都建出来，tidy 再删就是两个脚本互相抵消。2026-09-21 修：此前只护到
`extra_categories`，暂时没挂文献的子类会被「apply 建 → tidy 删 → apply 换 key 重建」。
...
- 期刊简写查 `state/journal_abbr.json`（全称 → ISO4 简写，**手工可改**）；
  ...
  ⚠️ **补表要查 CrossRef**，所以离线时拿不到缩写会用**刊名全称占位**写进表，而 `Abbr.add`
  只增不改——正因如此，**只报告模式只算不落盘**，落盘必须显式 `--apply`（2026-09-21 起
  默认路径也会补，此前只在 `--build-abbr` 时补，等于表永远补不上）。
...
额度重置后重跑 `journal_if.py` 即可补齐。2026-09-21 起分得更细：**表里已有旧值、只是这次
没查到**的刊进 `kept_old`，**429 收工时根本没查过**的刊进 `not_queried` —— 两者都不同于
「查过、确实没有 IF」。`--refresh` 现在也读旧表，撞上 429 提前收工不会再拿残缺的表覆盖
整份文件（旧写法会让其余 60 来个刊的 IF 全丢、日报 `J'` 对它们变 0）。已修的取错源问题：
`Angewandte Chemie` 原先取到
...
**前置**：`state/library_index.json`（`library_index.py`）、`state/citation_graph.json`
...
排序 = 数值排序——**负分不满足**（`-0.100` 是 6 字符，且负数之间字符串序与数值序**相反**），
所以底部那几篇在排序里位置是倒的，别把 Extra 列当严格数值序用。
同键的行**原地替换**（`upsert_line` 自己实现，
