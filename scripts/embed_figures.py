...
请挑出 **4–6 张**对论文论证最重要的图，把结果写入 figures.plan.md。硬性下限是 **2 张**——
只挑得出 2 张就写 2 张；一张都挑不出才算失败。宁少勿滥，但不要少于 2 张。
...
def process(rec):
    ...
    ctx_path = os.path.join(d, "figures.context.md")
    plan_path = os.path.join(d, "figures.plan.md")
    open(ctx_path, "w", encoding="utf8").write(ctx)
    for p in (plan_path,):
        if os.path.exists(p):
            os.remove(p)
    ...
    return key, f"ok({len(items)}图)", time.time() - t0, bad
