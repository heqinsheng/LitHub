...
（0.312 / 1.400 都是 5 字符）是为了让字符串排序等于数值排序。
...
def load_library(log):
    ...
    idx = json.loads((STATE / "library_index.json").read_text())
    cg = json.loads((STATE / "citation_graph.json").read_text())
    cls = json.loads((STATE / "classification.json").read_text())
    ...
    if pf.exists():
        weights = json.loads(pf.read_text()).get("weights") or {}
...
def main():
    ...
    def flush():
        with (LOGS / "score_library.log").open("a") as f:
            f.write("\n".join(lines) + "\n")
    ...
    # ── X：被引量。批量取（100 个 DOI 一批），按月缓存；这是唯一的新增网络开销
    tag = dd.period_tag(today_d, 30)
