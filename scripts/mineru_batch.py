#!/usr/bin/env python3
"""把 Zotero 里的 PDF 批量转成 Markdown（MinerU flash-extract）。

实测限制：
  - flash-extract 单次上限：10 MB 且 20 页
  - 文件大小限制先于页数限制触发，--pages 绕不过 10MB
对策：
...
def convert(rec):
    ...
    if not FORCE and os.path.exists(md_path) and os.path.getsize(md_path) > 800:
        ...
        head = open(md_path, encoding="utf-8", errors="replace").read(400)
        if f"engine: {ENGINE}" in head:
            return key, "skip", 0
