...
AGENT_FILE = OPTS.get("agent-file") or os.path.join(
...
def load_template(path):
    ...
    if "{meta}" not in body:
        log(f"⚠ 提示词模板 {path} 里没有 {{meta}} 占位符，改用内置提示词")
        return PROMPT, FIGURE_ADDENDUM
...
# 裁剪后的全文塞进命令行参数的上限。Linux 单个参数上限是 128 KB（MAX_ARG_STRLEN），
# 留出模板 + 元数据 + 库内清单的余量；超了就改走临时文件（paper.summarize.md）让模型读一次。
ARGV_LIMIT = 110_000
...
def build_prompt(key, d, item, ab, tpl, mode=None):
    ...
    载体（mode）：
      ...
      file    塞不下（>110 KB）就写成本目录的 `paper.summarize.md`，让模型读一次再回答；
    ...
    if mode in ("inline", "file"):
        try:
            ...
            if mode == "inline" and len(trimmed.encode()) > ARGV_LIMIT:
                mode = "file"
            if mode == "inline":
                src = f"\n以下是论文全文：\n\n{trimmed}\n"
            else:
    ...
    if FIGURES and mode == "tool":   # 图片段要模型自己读图，只对工具模式有意义
        prompt += "\n\n" + figs.replace("{figure_manifest}", figure_manifest(d, MAX_FIGURES))
    return prompt.replace("{figure_manifest}", ""), note, mode, tmp
...
def preflight(todo):
    ...
    if not DRY and any("控制字节" in pr for _k, pr in bad):
        ...
        log(f"  → 控制字节修复：{tail[0][:80]}")
        bad = [(k, [x for x in pr if x != "控制字节"] or ["控制字节（本轮已修）"]) for k, pr in bad]
