...
ORDER += [k for k in sorted({c["primary"] for c in cls} - set(ORDER))]
...
pc = collections.Counter(c["primary"] for c in cls)
...
L.append("|---:|---|---|---|:--:|:--:|---|---|---|---|---:|---|---|")
for i, c in enumerate(sorted(cls, key=lambda x: (ORDER.index(x["primary"])
                                                 if x["primary"] in ORDER else 99,
                                                 x.get("year", ""))), 1):
    L.append("| {} | {} | {} | {} | {} | {} | {} | {} | {} | {} | {} | {} | {} |".format(
        i, c["primary"], c.get("secondary") or "—",
        ...
        c.get("year", "?"), str(c.get("title", "")).replace("|", "/")[:70], c["key"]))
L.append("")

L.append("\n## 3. 按一级分类分组\n")
for k in ORDER:
    grp = [c for c in cls if c["primary"] == k]
    if not grp:
        continue
    L.append(f"\n### {k}（{len(grp)} 篇）\n")
    for c in sorted(grp, key=lambda x: x.get("year", "")):
        L.append(f"- **[{c['key']}]** {str(c.get('title', ''))[:88]} ({c.get('year', '?')})")
    L.append("")
