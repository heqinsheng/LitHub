...
产出：
  state/journal_if.json
    {generated, metric, count, with_if, missing[], by_name{白名单全名: IF},
     by_norm{归一化全名: IF}, alias{缩写归一化写法: 白名单全名}}
...
def build(refresh=False, log=print):
    ...
    if OUT.exists() and not refresh:
        try:
            old = json.loads(OUT.read_text()).get("by_name") or {}
        except Exception:
            old = {}
    by_name, missing, n_net = {}, [], 0
    ...
    for i, w in enumerate(names, 1):
        ...
        if _RATE_LIMITED["hit"]:
            log(f"  ! OpenAlex 额度耗尽，在第 {i}/{len(names)} 个刊处收工；"
                f"已查到 {len(by_name)} 种。额度次日 00:00 UTC 重置，重跑本脚本即可补齐")
            break
...
def main():
    ...
    if out.get("rate_limited"):
        print(f"注意：OpenAlex 每日额度已耗尽（retry-after {out['rate_limited']} 秒，"
              f"次日 00:00 UTC 重置），本次是提前收工——上面的「查不到」里有一部分其实是"
              f"没查到，不是真的没有；额度重置后重跑本脚本即可补齐。")
