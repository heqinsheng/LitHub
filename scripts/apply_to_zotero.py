...
def api(method, path, body=None):
    req = urllib.request.Request(
        ...
        headers={"Zotero-API-Version": "3", "Content-Type": "application/json",
                 ...
                 "Authorization": f"Bearer {open(KEYFILE).read().strip()}"})
    try:
...
def main():
    ...
    cls = json.load(open(os.path.join(ROOT, "state", "classification.json")))
    ...
    off = {}
    for c in cls:
        bad = [f"{ax}:{v}" for ax, field in FIELDS.items()
    ...
    if off:
        ...
        for k, v in off.items():
            print(f"      {k}: {'、'.join(v)}")
    if any(c.get("coupling") for c in cls):
        unknown = {cp for c in cls for cp in (c.get("coupling") or [])
                   if cp not in COUPLING}
        if unknown:
            print(f"  ! 未知耦合标签，已跳过: {sorted(unknown)}")
