...
三个通道（同一套检索式，各自过闸后统一打分）：
  fresh  Crossref 近 --days 天新入库的 journal-article（--rows 100 × --pages 1，
         即每个检索式 100 条）。窗口固定为 --days，凑不满由另外两个通道补，
         不再拉长窗口用旧文冒充新文。fresh 的缓存键含 since（每天变），所以每期
         必然重取 40 个请求（检索式数 × 页数；2026-09 补过力学/腐蚀检索式后从 27 升到
         40）——页数是每日耗时的主因，默认 1 页；
         rows 保持 100 是因为耗时虽随 rows 涨，但请求数固定，100 条换来的候选是
         50 条的两倍（实测 16–21 篇 vs 5–7 篇），日耗时仍在预算内。
...
成本控制：
  ...
  - 每日稳定态约 4 分钟（实测 250 s：fresh 40 个冷请求 + 其余命中缓存）；
    定 A 用的 Unpaywall 查询比旧版多（短名单 22 + 全局前 48，去重后实测量 55–63 篇），
    首次约 +30–50 s，之后命中 state/work/cache_unpaywall 零成本
  - 跨刷新周期（pool.refresh_days，默认 30 天）的首次运行要多取 query 的 120 个请求
    + refs 的 8 个批次，加上 fresh 的 40 个冷请求，整次约 10–12 分钟（周期一次；
    27 条检索式时实测 query 那 81 个请求 423 s，现在 40 条会更久）
...
def landing_absolute(doi):
    ...
    if p.exists():
        try:
            got = json.loads(p.read_text()).get("abstract", "")
...
def translate(items, model, log):
    ...
    try:
        import tomllib
        cfg = tomllib.loads((Path.home() / ".kimi-code" / "config.toml").read_text())
...
def last_push_date():
    ...
    """
    try:
        v = json.loads(LAST_RUN.read_text()).get("last_push") or ""
...
def load_library(log):
    ...
    if LIB_INDEX.exists():
        for x in json.loads(LIB_INDEX.read_text()).get("items", []):
            if x.get("norm_title"):
    ...
    try:
        for x in json.loads((STATE / "digest.json").read_text()):
    ...
    for c in json.loads((STATE / "classification.json").read_text()):
...
def load_recs():
    ...
    if RECS_LEGACY.exists():
        try:
            for d, dt in json.loads(RECS_LEGACY.read_text()).items():
                if d in recs:
                    continue
...
def main():
    ...
    ap.add_argument("--rows", type=int, default=_rt.POOL["rows"],
                    help="Crossref 每页条数（默认 100；实测单请求 100 条约 7.9s、50 条约 3.0s，"
                         "但 fresh 每期必冷的请求数是 40 个（检索式数×页数），100 条的日耗时"
                         "约 4.5 分钟、仍在预算内，而 50 条只捞回一半候选；"
                         "默认值来自 config/runtime.json）")
    ap.add_argument("--pages", type=int, default=_rt.POOL["pages"],
                    help="fresh 通道页数（默认 1；fresh 每期必冷，页数是耗时的主因；"
                         "默认值来自 config/runtime.json）")
    ...
    ap.add_argument("--cat-floor", default=_rt.DIGEST["cat_floor"],
                    help="分类保底席位，「分类=整数」逗号分隔，空串关闭。保底在通道配额轮"
                         ...
                         "与每类上限 cap。默认值复现的是「R 去掉 w_cat 之前」的实测构成"
                         "（力学耦合 3、界面反应 2；默认值来自 config/runtime.json）")
    ...
    if not queries:
        ...
        queries = getattr(fc, "QUERIES", {c: [] for c in fc.QUERIES})
    else:
    ...
    if CITE_GRAPH.exists():
        cg = json.loads(CITE_GRAPH.read_text())
    ...
    if a.dry_run:
        ...
        (NEWS / f"{today}.md").write_text(text)
        (NEWS / f"{today}.urls.txt").write_text(urls)
        n_again = 0
        for r in picked:
            rec = recs.get(r["doi"]) or {}
            if rec.get("times_pushed"):
                n_again += 1
            recs[r["doi"]] = {
                "first_pushed": rec.get("first_pushed") or today,
                "last_pushed": today,
                "times_pushed": int(rec.get("times_pushed") or 0) + 1,
                "channel": r["channel"],
        ...
        log(f"  已写 文献日报/{today}.md 与 {today}.urls.txt；"
            ...
            f"（其中 {n_again} 条是冷却期外的再推，现共 {len(recs)} 条）；"
            f"state/last_push.json 记为 {today}")
