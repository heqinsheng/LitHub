...
def main():
    ...
    digest = {d["key"]: d for d in json.loads((STATE / "digest.json").read_text())}
    n = len(cls)
    ...
    # ══════════════════════════════════════════════════════════════════════
    # 画像第 7–9 节的数据：时间趋势 / 值得注意的发现 / 被引与推荐分
    ...
    for c in CATS:
        L.append(f"| {c} | {cat_cnt.get(c,0)} | {cat_cnt.get(c,0)/n*100:.0f}% | "
    ...
    for c in CATS:
        ...
        for q in queries[c]:
            L.append(f"- `{q}`\n")
    (ROOT / "文献画像.md").write_text("".join(L))
